package InterfacesAndAbstractions.armyFromLection.army.factories;

import InterfacesAndAbstractions.armyFromLection.army.*;
import InterfacesAndAbstractions.armyFromLection.army.enums.Corps;
import InterfacesAndAbstractions.armyFromLection.army.enums.State;
import InterfacesAndAbstractions.armyFromLection.army.helperClasses.MissionImpl;
import InterfacesAndAbstractions.armyFromLection.army.helperClasses.RepairImpl;
import InterfacesAndAbstractions.armyFromLection.army.helperClasses.interfaces.Mission;
import InterfacesAndAbstractions.armyFromLection.army.helperClasses.interfaces.Repair;
import InterfacesAndAbstractions.armyFromLection.army.interfaces.*;

import java.util.List;

public class SoldierFactory {



    public static Soldier produce(List<String> args) {
        return null;
    }

    public static Soldier producePrivate(List<String> args) {
        return new PrivateImpl(
                Integer.parseInt(args.get(0)),
                args.get(1),
                args.get(2),
                Double.parseDouble(args.get(3)));
    }


    public static Soldier produceLeiutenantGenaral(List<String> args, List<Soldier> soldiers){
        LieutenantGeneral general = new LieutenantGeneralImpl(
                Integer.parseInt(args.get(0)),
                args.get(1),
                args.get(2),
                Double.parseDouble(args.get(3))
        );

        for (int i = 4; i < args.size(); i++) {
             int id = Integer.parseInt(args.get(i));

            for (int j = 0; j < soldiers.size(); j++) {
                 if(soldiers.get(j).getId() == id){
                     general.addPrivate((Private) soldiers.get(j));
                 }

            }
        }
        return (Soldier) general;
    }


    public static Soldier produceEngineer(List<String> args){
        Engineer engineer = new EngineerImpl(
                Integer.parseInt(args.get(0)),
                args.get(1),
                args.get(2),
                Double.parseDouble(args.get(3)),
                Corps.valueOf(args.get(4).toUpperCase())
        );

        for (int i = 5; i < args.size(); i+=2) {
            Repair repair = new RepairImpl(args.get(i), Integer.parseInt(args.get(i+1)));
            engineer.addRepai(repair);
        }

        return (Soldier) engineer;
    }


    public static Soldier produceCommando(List<String> args){
        Commando commando = new CommandoImpl(
                Integer.parseInt(args.get(0)),
                args.get(1),
                args.get(2),
                Double.parseDouble(args.get(3)),
                Corps.valueOf(args.get(4).toUpperCase())
        );

        for (int i = 5; i < args.size(); i+=2) {
            Mission mission = new MissionImpl(args.get(i), State.valueOf(args.get(i+1).toUpperCase()));
            commando.addMission(mission);
        }

        return (Soldier) commando;
    }



    public static Soldier produceSpy(List<String> args){
        return new SpyIml(
                Integer.parseInt(args.get(0)),
                args.get(1),
                args.get(2),
                args.get(3));
    }
}
